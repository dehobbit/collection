...
int main(int argc, char* argv[]) {
	...
	//Debug Check if there is an assembly file
	/*	if (argc != 2){
		...
		if (!instructionLine.empty()) {
			...
			if (token.size() != 0) {
				...
				if (command.find(token[0]) == command.end() && type.find(token[0]) == type.end()) {
					//not a command but label or comment
					if (token[0][0] == ';')
						break;
					//check label redundancy
				...
				if (command.find(token[startPosition]) != command.end()) { //instruction
	...
	while (running) {
		...
		case 0: //trap
			...
			case 3: //write character to standard out
				cout << (char)reg[7];
				break;
			case 4: //read character from standard in
				reg[7] = getchar();
				break;
...
bool checkInstructionSyntax(vector<string> token, int start) {
	...
	if (token[start] == "ADD" || token[start] == "SUB" || token[start] == "DIV" || token[start] == "MUL" || token[start] == "MOV" || token[start] == "CMP"
		...
		if (!isReg(token[start + 1])) return false;
		if (!isReg(token[start + 2])) return false;
		return true;
	}
	if (token[start] == "ADI") {
		if ((token.size() - start) != 3) return false;
		if (!isReg(token[start + 1])) return false;
		if (!isInt(token[start + 2])) return false;
		return true;
	}
	...
	if (token[start] == "LDB" || token[start] == "LDR" || token[start] == "STB" || token[start] == "STR") {
		if ((token.size() - start) != 3) return false;
		if (!isReg(token[start + 1])) return false;
		if (!isReg(token[start + 2]) && !isLabel(token[start + 2])) return false;
		return true;
	}
	...
	if (token[start] == "TRP") {
		if ((token.size() - start) != 2) return false;
		if (!isInt(token[start + 1])) return false;
		return true;
	}
...
bool generateByteCode(vector<string> token, char* instr, int instrPos, map<string, int>& symbol, int start) {
	...
	if (token[start] == "ADD" || token[start] == "SUB" || token[start] == "DIV" || token[start] == "MUL" || token[start] == "MOV" ||
		...
		if (!isReg(token[start + 1])) return false;
		if (!isReg(token[start + 2])) return false;
		int destinationRegister = getReg(token[start + 1]);
		int sourceRegister = getReg(token[start + 2]);
